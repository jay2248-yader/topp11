  ...
  const handleSave = async () => {
    try {
      ...
      setGroups(gs => gs.map(g => 
        g.id === editingGroupId ? { ...g, ...updates } : g
      ));
  ...
  <Link to="/topp8/admin" className="back-button">
    &larr; ກັບໜ້າຫຼັກ
      ...
      <div className="manage-groups-topic-select">
        <label>ເລືອກຫົວຂໍ້:</label>
        <select
          value={selectedTopicId || ""}
          onChange={e => setSelectedTopicId(e.target.value)}
        >
          <option value="" disabled>-- ເລືອກຫົວຂໍ້ --</option>
          {topics.map(t => (
            ...
            editingGroupId === group.id ? (
              <li key={group.id} className="manage-groups-edit-item">
                ...
                <div className="manage-groups-action-btns">
                  <button className="edit-button" onClick={handleSave}>ບັນທຶກ</button>
                  <button className="delete-button" onClick={() => setEditingGroupId(null)}>ຍົກເລີກ</button>
                </div>
              </li>
            ) : (
...
<ul className="manage-groups-members">
  {group.members.map((member, idx) => (
    <li key={idx} className="member-item">{member}</li>
  ))}
</ul>
                <p>ສະຖານະ: {group.status}</p>
                <div className="manage-groups-actions">
                  <button className="edit-button"onClick={() => handleEdit(group)}>ແກ້ໄຂ</button>
                  <button className="delete-button" onClick={() => handleDelete(group.id)}>ລົບ</button>
                </div>
              </li>
            )
          )}
